...
class ImageProcessor:
    ...
    def validate_image(self, image_data: bytes, content_type: str) -> Tuple[bool, str]:
        ...
        if content_type not in self.supported_formats:
            return (
                False,
                f"Unsupported format: {content_type}. Supported: {list(self.supported_formats.keys())}",
            )
        ...
        if len(image_data) > max_bytes:
            return (
                False,
                f"Image too large: {len(image_data)} bytes. Maximum: {max_bytes} bytes",
            )
        ...
        try:
            ...
            # Check dimensions
            if (
                image.width > settings.MAX_IMAGE_DIMENSION
                or image.height > settings.MAX_IMAGE_DIMENSION
            ):
                return (
                    False,
                    f"Image dimensions too large: {image.width}x{image.height}. Maximum: {settings.MAX_IMAGE_DIMENSION}x{settings.MAX_IMAGE_DIMENSION}",
                )
    ...
    def rgba_to_rgb_for_model(
        self, rgba: np.ndarray, bg_color: Tuple[int, int, int] = (128, 128, 128)
    ) -> np.ndarray:
    ...
    async def generate_masks(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Generator done job_id={job_id} op_id={op_id} elapsed={time.perf_counter()-start:.2f}s masks={len(sam_result)}"
                )
            finally:
                done = True
                try:
    ...
    async def segment_image(
        ...
        if predictor is None:
            ...
            predictor = await model_manager.load_model(
                model_type or settings.DEFAULT_MODEL
            )
        ...
        if boxes or points:
            ...
            if boxes:
                for b in boxes:
                    ...
                    x = (
                        int(b.get("x", b.get("x1", 0)) * w)
                        if 0 <= b.get("x", b.get("x1", 0)) <= 1
                        else int(b.get("x", b.get("x1", 0)))
                    )
                    y = (
                        int(b.get("y", b.get("y1", 0)) * h)
                        if 0 <= b.get("y", b.get("y1", 0)) <= 1
                        else int(b.get("y", b.get("y1", 0)))
                    )
                    width = (
                        int(b.get("width", max(0, b.get("x2", 0) - b.get("x1", 0))) * w)
                        if b.get("width") is not None
                        else (
                            int(max(0, b.get("x2", 0) - b.get("x1", 0)) * w)
                            if 0 <= b.get("x1", 0) <= 1
                            else int(max(0, b.get("x2", 0) - b.get("x1", 0)))
                        )
                    )
                    height = (
                        int(
                            b.get("height", max(0, b.get("y2", 0) - b.get("y1", 0))) * h
                        )
                        if b.get("height") is not None
                        else (
                            int(max(0, b.get("y2", 0) - b.get("y1", 0)) * h)
                            if 0 <= b.get("y1", 0) <= 1
                            else int(max(0, b.get("y2", 0) - b.get("y1", 0)))
                        )
                    )
                    box_xyxy = np.array([x, y, x + width, y + height])
                    masks, scores, logits = predictor.predict(
                        box=box_xyxy, multimask_output=multimask_output
                    )
                        ...
                        mask_items.append(
                            {
                                ...
                                "predicted_iou": (
                                    float(scores[idx]) if scores is not None else 1.0
                                ),
                                "stability_score": (
                                    float(scores[idx]) if scores is not None else 1.0
                                ),
            ...
            logger.info(
                f"Segmentation with boxes produced {len(mask_items)} masks job_id={job_id}"
            )
            return mask_items
        ...
        if torch.cuda.is_available():
            ...
            effective_params["points_per_side"] = int(
                effective_params.get("points_per_side", 16)
            )
        logger.info(
        ...
        masks = await self.generate_masks(
            ...
            crop_n_points_downscale_factor=int(
                params.get("crop_n_points_downscale_factor", 2)
            ),
        ...
        logger.info(
            f"Progress 90% job_id={job_id} step=masks_generated count={len(masks)}"
        )
        try:
            import psutil
            ...
            logger.info(
                f"Resource snapshot location=ImageProcessor.segment_image stage=post_generate rss_mb={rss_mb:.2f} job_id={job_id}"
            )
        except Exception:
            pass
        logger.info(
            f"Segmentation completed in {perf_counter() - t0:.2f}s with {len(masks)} masks job_id={job_id}"
        )
        return masks
    ...
    def create_visualization(
        ...
        try:
            ...
            for i, mask_data in enumerate(top_masks):
                mask = self._base64_to_mask(
                    mask_data["segmentation"], original_image.shape[:2]
                )
                try:
                ...
                color_overlay = Image.new(
                    "RGBA", original_pil.size, colors[i] + (128,)
                )  # 50% transparency
                ...
                if with_labels:
                    try:
                        ...
                        if len(xs) and len(ys):
                            ...
                            overlay.paste(
                                bg, (max(0, cx - tw // 2), max(0, cy - th // 2))
                            )
                            draw.text(
                                (cx - 4, cy - 8), label_text, fill=(255, 255, 255, 255)
                            )
                    except Exception:
                        pass
    ...
    def _base64_to_mask(self, base64_str: str, shape: Tuple[int, int]) -> np.ndarray:
        ...
        if mask_array.shape != shape:
            mask_array = cv2.resize(
                mask_array, (shape[1], shape[0]), interpolation=cv2.INTER_NEAREST
            )
    ...
    def generate_plain_shirts(
        ...
        try:
            ...
            for m in masks:
                arr = self._base64_to_mask(
                    m.get("segmentation") or m.get("mask"), (h, w)
                )
                area = int(np.sum(arr.astype(np.uint8)))
                decoded.append({"mask": arr, "area": area})
            if not decoded:
                return []
            shirts = sorted(decoded, key=lambda x: x["area"], reverse=True)[:2]
            rgb = image_rgba[:, :, :3].copy()
            alpha = image_rgba[:, :, 3].copy()
            results: List[np.ndarray] = []
            for s in shirts:
                S = (s["mask"] > 0).astype(np.uint8) * 255
                s_area = max(1, int(s["area"]))
                logo_mask = np.zeros_like(S, dtype=np.uint8)
                for d in decoded:
                    if d is s:
                        continue
                    if int(d["area"]) < int(s_area * ratio_thresh):
                        m = (d["mask"] > 0).astype(np.uint8) * 255
                        m = cv2.bitwise_and(m, m, mask=S)
                        logo_mask = cv2.bitwise_or(logo_mask, m)
                if np.count_nonzero(logo_mask) > 0:
                    ksz = max(3, int(round(inpaint_radius * 2 + 1)))
                    kernel = np.ones((ksz, ksz), np.uint8)
                    Ld = cv2.dilate(logo_mask, kernel, iterations=1)
                    ring = cv2.bitwise_and(Ld, cv2.bitwise_not(logo_mask))
                    ring = cv2.bitwise_and(ring, S)
                    idx = ring > 0
                    if np.count_nonzero(idx) > 0:
                        ref = rgb[idx]
                        med = np.median(ref, axis=0).astype(np.uint8)
                        filled = rgb.copy()
                        filled[logo_mask > 0] = med
                        filled = cv2.bilateralFilter(
                            filled, d=9, sigmaColor=75, sigmaSpace=75
                        )
                        shirt_rgb = np.where(S[..., None] > 0, filled, rgb)
                    else:
                        bgr = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
                        inp = cv2.inpaint(
                            bgr, logo_mask, inpaint_radius, cv2.INPAINT_TELEA
                        )
                        inpainted = cv2.cvtColor(inp, cv2.COLOR_BGR2RGB)
                        shirt_rgb = np.where(S[..., None] > 0, inpainted, rgb)
                else:
                    shirt_rgb = rgb
                # Flatten shirt luminance for consistent presentation
                try:
                    idx = S > 0
                    if np.count_nonzero(idx) > 0:
                        base = np.median(shirt_rgb[idx], axis=0).astype(np.uint8)
                        shirt_rgb[idx] = base
                except Exception:
                    pass
                rgba = np.dstack(
                    [shirt_rgb[:, :, 0], shirt_rgb[:, :, 1], shirt_rgb[:, :, 2], alpha]
                )
                ys, xs = np.where(S > 0)
                if len(xs) and len(ys):
                    y0, y1 = int(ys.min()), int(ys.max())
                    x0, x1 = int(xs.min()), int(xs.max())
                    results.append(rgba[y0:y1, x0:x1])
            return results
        except Exception as e:
            logger.error(f"Failed to generate plain shirts: {str(e)}")
            return []
